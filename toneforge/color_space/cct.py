...
def getMatrixCCT_from_anywhere(M_lms, Wp, Td):
    def getWhitePointXYZ(T):
        x, y = WhitePoint[T]
        # rgb = np.matmul(np.linalg.inv(M), np.array([[x/y],[1],[(1-x-y)/y]]))
        # rgb = rgb + np.maximum(0, -np.min(rgb))
        ...
        return np.array([[x/y],[1],[(1-x-y)/y]])
    x,y = Wp
    Ts_xyz = np.array([[x/y],[1],[(1-x-y)/y]])

    scale = np.eye(3) * np.matmul(M_lms, getWhitePointXYZ(Td)) / np.matmul(M_lms, Ts_xyz)
    return np.matmul(np.linalg.inv(M_lms), np.matmul(scale, M_lms))

def getMatrixCCT2(M_lms, Ws, Wd):
    def getWhitePointXYZ(Wp):
        x, y = Wp
        # rgb = np.matmul(np.linalg.inv(M), np.array([[x/y],[1],[(1-x-y)/y]]))
        # rgb = rgb + np.maximum(0, -np.min(rgb))
        # rgb = rgb / np.max(rgb)
        # return np.matmul(M, rgb)
        return np.array([[x/y],[1],[(1-x-y)/y]])
    scale = np.eye(3) * np.matmul(M_lms, getWhitePointXYZ(Wd)) / np.matmul(M_lms, getWhitePointXYZ(Ws))
    return np.matmul(np.linalg.inv(M_lms), np.matmul(scale, M_lms))